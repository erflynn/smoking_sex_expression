...
gsm_dat <- dbGetQuery(con, "SELECT gsm, molecule_ch1, title, source_name_ch1,
                      ...
                      WHERE organism_ch1 IN ('Homo sapiens', 'Mus musculus');")
...
# 568 studies, 37,797 samples

gsm_gse2 <- gsm_gse0 %>% bind_rows(gsm_gse) %>% distinct(gse, gsm)
# 712 studies, 48,566 samples
...
sample_counts <- gsm_gse3 %>% distinct(gse, gsm) %>% group_by(gse) %>% summarize(num_samples=n())
...
geo_smoking_studies2 %>% write_csv("data/geo_smok_studies_1108.csv")
...
filtered_ae <- ae_experiments %>% 
  ...
  filter(str_detect(Organism, "Mus musculus") |
           str_detect(Organism, "Homo sapiens")) %>% # 411
  filter(!grepl("miR|MiR|microRNA|MicroRNA", Title, ignore.case=F)) # 372

filtered_ae2 <- filtered_ae %>% 
  mutate(Accession=str_replace_all(Accession, "E-GEOD-", "GSE"))

ae_gses <-filtered_ae2 %>% filter(str_detect(Accession, "GSE")) %>% pull(Accession) # 276
length(intersect(ae_gses, geo_smoking_studies$gse)) # 239 intersect
length(setdiff(ae_gses, geo_smoking_studies$gse)) # 37 intersect
...
sum(array_exp_studies$num_samples) # 61153 samples, 133 studies
stopifnot(nrow(array_exp_studies)==length(unique(array_exp_studies$study_acc)))
array_exp_studies %>% write_csv("data/array_exp_studies_1108.csv")
...
sra_mouse <- read_csv("data/metaSRA-mouse_samples.csv")

metasra_studies_h <- sra_human %>% 
  group_by(study_id, study_title) %>% 
  summarize(num_samples=n()) %>%
  mutate(organism="Homo sapiens") # 24
metasra_studies_m <- sra_mouse %>% 
  group_by(study_id, study_title) %>% 
  summarize(num_samples=n()) %>%
  mutate(organism="Mus musculus") # 2
metasra_studies <- metasra_studies_h %>% 
  bind_rows(metasra_studies_m) %>%
...
m_rb_metadata <- read_csv("../drug_trt/data/01_sample_lists/rb_metadata/mouse_rnaseq_experiment_metadata.csv")
h_rb_smok <- h_rb_metadata %>%
  filter(grepl(SMOK.STR, title, ignore.case = T) |
           grepl(SMOK.STR, description, ignore.case = T)) # 37
m_rb_smok <- m_rb_metadata %>%
  filter(grepl(SMOK.STR, title, ignore.case = T) |
           grepl(SMOK.STR, description, ignore.case = T)) # 14

#h_rb_smok %>% bind_rows(m_rb_smok) %>% anti_join(metasra_studies, by="study_acc")  # 41


# C) Look at RNA-seq covariate data
load("../drug_trt/data/data_old/01_sample_lists/all_sample_attrib_clean.RData") # --> all_attrib_clean
length(unique(all_attrib_clean$sample_acc))

rb_smok_attr <- all_attrib_clean %>% 
  filter(str_detect(key_clean, SMOK.STR) |  
           str_detect(value_clean, SMOK.STR)) # 12509 samples
rb_smok_attr_rnaseq <- rb_smok_attr %>%
  filter(str_detect(sample_acc, "ERR|SRR|DRR")) # 1809 samples

rb_exp_sample_map <- list.files(path="../drug_trt/data/01_sample_lists/rb_metadata/", 
                                pattern="*_exp_to_sample.csv") %>%
  map(~read_csv(sprintf("../drug_trt/data/01_sample_lists/rb_metadata/%s", .))) %>%
  bind_rows()
...
length(setdiff(unique(all_attrib_clean$sample_acc), sample_map$sample_acc)) # 1
rb_exp_sample <-sample_map %>%
  semi_join(rb_smok_attr, by="sample_acc")
rb_exp <- rb_exp_sample %>% 
  group_by(study_acc) %>% 
  summarize(num_samples=n()) # --> 230 studies

sra_rb_exp <- rb_exp %>% 
  filter(str_detect(study_acc, "ERP|SRP|DRP")) # 41
# --> adds one AE study
...
sra_study_acc <- metasra_studies %>% select(study_acc) %>%
  bind_rows(sra_rb_exp %>% select(study_acc)) %>%
  bind_rows(h_rb_smok %>% select(study_acc)) %>%
  bind_rows(m_rb_smok %>% select(study_acc)) %>%
  distinct(study_acc) # 80

# put together a table
rb_metadata <- h_rb_metadata %>%
  mutate(organism="Homo sapiens") %>%
  bind_rows(m_rb_metadata %>% mutate(organism="Mus musculus")) %>%
  select(-date)
setdiff(sra_study_acc$study_acc, rb_metadata$study_acc) 
# good -- all contained, which means we have all this info
...
sra_sample_counts <- rb_exp_sample_map %>% 
  ...
  group_by(study_acc) %>%
  summarize(num_samples=n())
sra_metadata <- sra_metadata_filt %>% left_join(sra_sample_counts, by=c("study_acc"))
sum(sra_metadata$num_samples)
...
sra_metadata %>% write_csv("data/sra_studies_1108.csv")
...
combined_data <- geo_smoking_studies2 %>% 
  ...
  mutate(organism=case_when(
    str_detect(organism, "Homo sapiens") &
      str_detect(organism, "Mus musculus") ~ "Homo sapiens; Mus musculus",
    str_detect(organism, "Homo sapiens") ~ "Homo sapiens",
    str_detect(organism, "Mus musculus") ~ "Mus musculus",
    TRUE ~ ""
  ))

nrow(combined_data) # 823
...
study_counts %>% write_csv("data/study_counts_before_filtering_1108.csv")
...
sample_counts %>% write_csv("data/sample_counts_before_filtering_1108.csv")

# ---- 5. Add in previous notes ----- #

prev_manual_annot <- read_csv("data/smok_data_manual_annot_0408.csv")  %>% rename(study_acc=gse) # 274
combined_data %>% semi_join(prev_manual_annot, by="study_acc")  %>% nrow() # 267
combined_data %>% anti_join(prev_manual_annot, by="study_acc")  %>% nrow() # 556
...
combined_data_w_annot %>% write_csv("data/manual_annot_1108.csv")
