...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE42057_series_matrix.txt.gz)
...
p1_gpl570 %>% 
  select(title, geo_accession, characteristics_ch1.6) %>% 
  View()

smok_vars <- p1_gpl570 %>% 
  select("geo_accession", "title", "characteristics_ch1.6") %>%
  rename(smoker="characteristics_ch1.6")#, copd="characteristics_ch1.4")
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(smok_vars$smoker, "smokcignow") ~ "smoker"
