...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE12585_series_matrix.txt.gz)
...
smok_vars <- p1_gpl570 %>% 
  select(geo_accession, "title", "characteristics_ch1", "characteristics_ch1.1") %>%
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(smok_vars$smoker, "smoker") ~ "smoker",
