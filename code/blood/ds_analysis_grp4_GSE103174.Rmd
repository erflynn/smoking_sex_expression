...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE103174_series_matrix.txt.gz)
...
smok_vars <- p1_gpl570 %>% 
  select(geo_accession, "title", "characteristics_ch1.1", "characteristics_ch1.2", "characteristics_ch1.3", "characteristics_ch1.4") %>%
  rename(smoker="characteristics_ch1.4")#, copd="characteristics_ch1.4")
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(smok_vars$smoker, "no") ~ "nonsmoker",
    str_detect(smok_vars$smoker, "yes") ~ "smoker",
))
...
smok_vars2 <- smok_vars2 %>%
  mutate(copd = case_when(
    str_detect(smok_vars2$copd, "Healthy") ~ "no",
...
#  filter(smok_vars2$copd == "no") %>% 
  #select(-copd)     

smok_healthy <- smok_vars2
#smok_healthy <- filter(smok_vars2, copd == "no")
...
#summary(p1_gpl570)
covariate <- p1_gpl570 %>% 
  select(geo_accession, "lung t cells:ch1", "pack years:ch1", "Sex:ch1", "bmi:ch1", "copd:ch1" ) %>%
  rename(lung_t="lung t cells:ch1", pack_years="pack years:ch1", sex="Sex:ch1", bmi="bmi:ch1", copd="copd:ch1")
...
covariate %>%
  ...
  count() %>%
  arrange(desc(n))
table(covariate$pack_years)
table(is.na(covariate$pack_years)) # missing 205, this doesnt seem useful, but worth looking at to try to figure out what is going on?
...
1# if there is less than 5 in a category, make note of it on the `tissue_info` spreadsheet in a column, write out this info but we may need to discard for expression analysis
```
