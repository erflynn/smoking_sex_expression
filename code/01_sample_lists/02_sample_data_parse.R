...
# TODO:
# - issue: we are parsing metadata from multiple sources!
#    - one is geometadb
#    - the other is "all_attrib_clean": how did we get this??
#       - do they have different info?
...
find_col_stats <- function(df){
  df1 <- df %>%
    mutate(attr=tolower(attr),
           value=tolower(value)) #%>%
  #mutate(value=ifelse(is.na(value) | value=="", "-999", value))
  df2 <- df1 %>%
    unique() %>%
  ...
  age_cols <- find_col(df1, df2, "\\bage")
  pkyrs_cols <- find_col(df1, df2, "pack|pkyr|pyr")
  ...
  df4 <- df2.5 %>% group_by(sample_acc) %>% mutate(across(names(col_list), condense_dedup))
...
cleaned_dat <- do.call(rbind, lapply(col_stats, function(x) x$df_clean %>% 
                                       select(study_acc, sample_acc, "smok", "pkyrs",
                                              "sex", "race_ethnicity", "age")))


cleaned_dat2 <- cleaned_dat %>% 
  mutate(across(smok:age, ~ifelse(. %in% c("-999", "", "NA"), NA, .))) 
cleaned_dat2 %>%
  group_by(smok) %>%
  count() %>%
  arrange(desc(n)) %>%
...
summary(cleaned_dat2 %>%
          mutate(across(smok:race_ethnicity, as.factor), age=as.numeric(age)))


# do the columns vary
no_vary <- cleaned_dat2 %>%
  distinct(gse, smok) %>% group_by(gse) %>% count() %>%
  filter(n==1)

cleaned_dat2 %>% semi_join(no_vary) %>% distinct(gse, smok) %>% as_tibble()
lapply(unlist(tmp$cols), function(x) length(unique(tmp$df %>% pull({{x}}))))
...
aes_cleaned_dat <- do.call(rbind, lapply(aes_stats, function(x) x$df_clean %>% 
                                       select(study_acc, sample_acc, "smok", "pkyrs",
                                       "sex", "race_ethnicity", "age")))

aes_cleaned_dat2 <- aes_cleaned_dat %>% 
  mutate(across(smok:age, ~ifelse(. %in% c("-999", "", "NA"), NA, .))) 

# put data together
cleaned_dat_comb <- cleaned_dat2 %>% 
  #dplyr::rename(study_acc=gse, sample_acc=gsm) %>%
  bind_rows(aes_cleaned_dat2)
