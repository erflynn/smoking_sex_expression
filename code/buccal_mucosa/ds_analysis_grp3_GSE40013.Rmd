...
# This is for buccal mucosa study (GSE40013)
...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE40013_series_matrix.txt.gz)
...
p1_gpl570 %>% # grabs relevant columns
  select(geo_accession, characteristics_ch1.1, characteristics_ch1.2, characteristics_ch1.3, characteristics_ch1.4, characteristics_ch1.5) %>% 
  View()
...
smok_vars <- p1_gpl570 %>% 
  select(geo_accession, "title", "characteristics_ch1.1", "characteristics_ch1.2", "characteristics_ch1.3", "characteristics_ch1.4", "characteristics_ch1.5") %>%
  rename(smoker="characteristics_ch1.5")
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(tolower(smok_vars$characteristics_ch1.4), "never") ~ "nonsmoker",
    str_detect(tolower(smok_vars$characteristics_ch1.4), "current") ~ "smoker",
    str_detect(tolower(smok_vars$characteristics_ch1.4), "former") ~ "smoker",
))
...
smok_vars2 <- smok_vars %>%
  mutate(smoker=case_when(
    ...
    smoker %in% c(str_detect(tolower(smok_vars$title), "current"), TRUE) ~ "smoker",
