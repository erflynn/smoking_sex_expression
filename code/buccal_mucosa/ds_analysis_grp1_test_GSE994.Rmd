...
p1_gpl570 %>% # grabs relevant columns
  select(title, geo_accession, characteristics_ch1.1, characteristics_ch1.2, characteristics_ch1.3, characteristics_ch1.4, characteristics_ch1.5) %>% 
  View()
...
smok_vars2 <- smok_vars %>%
  mutate(smoker, new_smoker = )

smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(tolower(smok_vars$smoker), "never") ~ "nonsmoker",
    str_detect(tolower(smok_vars$smoker), "current") ~ "smoker",
    str_detect(tolower(smok_vars$smoker), "former") ~ "smoker",
))
...
#smok_vars2 <- smok_vars %>%
  #group_by(smok_vars$geo_accession)) %>%
  #mutate(smoker = case_when(
    #str_detect(smoker, "NeverSmoker") ~ "nonsmoker"
  #)
  #%>%
  #ungroup())
...
#smok_vars2 <- smok_vars %>%
  #mutate(smoker = case_when(
    #smoker %in% c("NeverSmoker", "nonsmoker") ~ "nonsmoker",
    #smoker %in% c("CurrentSmoker", "smoker") ~ "smoker"
