...
# This is for SAE study (GSE43079)
...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$`GSE43079-GPL16419_series_matrix.txt.gz`)
...
smok_vars <- p1_gpl570 %>% 
  select(geo_accession, characteristics_ch1) %>%
  rename(smoker="characteristics_ch1")
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(smok_vars$smoker, "smoking status: S") ~ "smoker",
    !str_detect(smok_vars$smoker, "smoking status: S") ~ "nonsmoker"
))
