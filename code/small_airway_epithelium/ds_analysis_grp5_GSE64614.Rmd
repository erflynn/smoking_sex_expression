...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE64614_series_matrix.txt.gz)
...
p1_gpl570 %>% 
  select(title, geo_accession, `smoking status:ch1`) %>% 
  View()

smok_vars <- p1_gpl570 %>% 
  select("geo_accession", "title", "smoking status:ch1") %>%
  rename(smoker="smoking status:ch1")
...
#smok_vars2 <- smok_vars2[!(smok_vars2$copd=="yes"),]

smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(smok_vars$smoker, "NS") ~ "nonsmoker",
    str_detect(smok_vars$smoker, "S") ~ "smoker",
    str_detect(smok_vars$smoker, "nonsmoker") ~ "nonsmoker",
    str_detect(smok_vars$smoker, "nonsmoker") ~ "smoker"
))

#delete the copd (na) rows
smok_vars2 <- smok_vars2[-c(202, 203), ]
...
smok_vars2 <- smok_vars2 %>%
  mutate(copd = case_when(
    str_detect(smok_vars2$copd, "yes") ~ "yes",
    !(str_detect(smok_vars2$copd, "yes")) ~ "no"
))
...
smok_healthy %>% View()
smok_healthy <- filter(smok_healthy, copd != 'yes')
