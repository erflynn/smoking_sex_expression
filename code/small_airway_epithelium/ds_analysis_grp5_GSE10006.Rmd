...
# This is for SAE study (GSE10006)
...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE10006_series_matrix.txt.gz)
...
select_gpl1570 <- # grabs relevant columns
  select(p1_gpl570, geo_accession, characteristics_ch1, characteristics_ch1.1, characteristics_ch1.2, characteristics_ch1.3) %>%
  View()
...
smok_vars <- p1_gpl570 %>% 
  select(geo_accession, "title", "characteristics_ch1", "characteristics_ch1.1", "characteristics_ch1.2", "characteristics_ch1.3") %>%
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    ...
    !str_detect(smok_vars$smoker, "non-smoker") ~ "smoker"
))
