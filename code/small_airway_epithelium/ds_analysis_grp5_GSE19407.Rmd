...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE19407_series_matrix.txt.gz)
...
p1_gpl570 %>% 
  select(title, geo_accession, characteristics_ch1.3) %>% 
  View()

smok_vars <- p1_gpl570 %>% 
  select("geo_accession", "title", "characteristics_ch1.3") %>%
  rename(smoker="characteristics_ch1.3")#, copd="characteristics_ch1.4")
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    ...
    str_detect(smok_vars$smoker, "pack-years") ~ "smoker"
))
...
smok_vars2 <- smok_vars2 %>%
  mutate(copd = case_when(
    str_detect(smok_vars2$title, "COPD") ~ "yes",
    str_detect(smok_vars2$title, "Control") ~ "no",
))
