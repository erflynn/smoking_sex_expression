...
# This is for SAE study (GSE11952)
...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE11784_series_matrix.txt.gz)
...
#names(p1_gpl570)[names(p1_gpl570) == "smoking status:ch1"] <- "smoking_status"

p1_gpl570 %>% # grabs relevant columns
  select(geo_accession, characteristics_ch1.3) %>%
  View()


#write.csv(select_gpl1570, "/Users/anniechang/Desktop/Research/smoking_sl/p1_gse10006.csv")
...
smok_vars <- p1_gpl570 %>% 
  select(geo_accession, characteristics_ch1, characteristics_ch1.1, characteristics_ch1.2, characteristics_ch1.3) %>%
  rename(smoker="characteristics_ch1.3")

smok_vars %>% View()

# what is missing?
# what are the values?
table(smok_vars$smoker)
table(is.na(smok_vars$smoker)) # looks like absence means no? this is a bad way to encode data so we're going to do something else with it
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(smok_vars$smoker, "non-smoker") ~ "nonsmoker",
    str_detect(smok_vars$smoker, "Status: smoker") ~ "smoker",
    str_detect(smok_vars$smoker, "status: smoker") ~ "smoker",
    str_detect(smok_vars$smoker, "COPD") ~ "COPD"
))

smok_vars2 <- smok_vars2 %>%
  mutate(copd = case_when(
    str_detect(smok_vars2$copd, "copd status") ~ "yes",
    !str_detect(smok_vars2$copd, "copd status") ~ "no"
))
...
#smok_vars2 <- smok_vars %>%
 # mutate(smoker=case_when(
   # smoker %in% c(str_detect(tolower(smok_vars$smoker), "status: non-smoker"), TRUE) ~ "nonsmoker",
   # smoker %in% c(str_detect(tolower(smok_vars$smoker), "status: smoker"), TRUE) ~ "smoker"
    #smoker %in% c(str_detect(tolower(smok_vars$title), "former"), TRUE) ~ "smoker",
  #))
 # mutate(smoker=case_when(
    #smoker %in% c(str_detect(tolower(smok_vars$title)), "never"), TRUE) ~ "nonsmoker"
 # )


#smok_vars2 <- smok_vars %>%
  #mutate(smoker=case_when(
    #smoker %in% c(str_locate(smok_vars$title, "NeverSmoker"), "nonsmoker") ~ "nonsmoker",
    #smoker %in% c(str_locate(smok_vars$title, "CurrentSmoker"), "smoker") ~ "smoker",
    #smoker %in% c(str_locate(smok_vars$title, "FormerSmoker"), "smoker") ~ "smoker"
...
table(smok_vars2$smoker)

# now for this analysis, we want health nonsmoker vs smoker

smok_healthy <- smok_vars2 %>%
  select(geo_accession, characteristics_ch1, characteristics_ch1.1, characteristics_ch1.2, smoker) %>%
  filter(smoker != "COPD")
...
covariate <- p1_gpl570 %>% 
  select(geo_accession, characteristics_ch1, characteristics_ch1.1, characteristics_ch1.2, characteristics_ch1.3) %>%
  #select(geo_accession, "source_name_ch1", "characteristics_ch1.1", "characteristics_ch1.2") %>%
  rename(age = "characteristics_ch1", sex = "characteristics_ch1.1", eth = "characteristics_ch1.2", smoker="characteristics_ch1.3")
  #rename(source="source_name_ch1", smok="characteristics_ch1.1", sex=`characteristics_ch1.2`)
...
table(covariate$geo_accession)
table(is.na(covariate$geo_accession)) # missing 205, this doesnt seem useful, but worth looking at to try to figure out what is going on?
# it's possible someone is present only once...
table(covariate$smok) 
table(is.na(covariate$smok)) # missing 156 samples
...
dir.create(sprintf("/Users/anniechang/Desktop/Research/smoking_sl/data/grp_data/%s/", grp_id))
                  # data/grp_data/%s/", grp_id)) # the first time for each group

smok_w_sex_lab3 %>% left_join(covariate) %>%
