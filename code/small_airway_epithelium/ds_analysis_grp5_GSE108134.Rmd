...
# This is for SAE study (GSE108134)
...
# grab phenotype data
ls(gse.obj1)
p1_gpl570 <- pData(gse.obj1$GSE108134_series_matrix.txt.gz)
...
p1_gpl570 %>% # grabs relevant columns
  select(geo_accession, characteristics_ch1, characteristics_ch1.1, characteristics_ch1.2, smoking_status) %>%
  View()
...
smok_vars <- p1_gpl570 %>% 
  select(geo_accession, characteristics_ch1, characteristics_ch1.1, characteristics_ch1.2, smoking_status) %>%
  rename(smoker="smoking_status", copd = "characteristics_ch1")
...
smok_vars2 <- smok_vars %>%
  mutate(smoker = case_when(
    str_detect(smok_vars$smoker, "nonsmoker") ~ "nonsmoker",
    !str_detect(smok_vars$smoker, "nonsmoker") ~ "smoker"
))
...
smok_vars2 <- smok_vars2 %>%
  mutate(copd = case_when(
    !str_detect(smok_vars2$copd, "COPD") ~ "no",
    str_detect(smok_vars2$copd, "COPD") ~ "yes"
))
...
table(covariate$source)
table(is.na(covariate$source)) # missing 205, this doesnt seem useful, but worth looking at to try to figure out what is going on?
