...
This is the example 
```{r}
gse.obj <- GEOquery::getGEO("GSE64614") 
...
names(pData_sm) <- c("geo_accession", "source_name", "copd", "ethnic", "vitD", "smok")
  
# clean up smoking status
pData_sm2 <- pData_sm %>%
  rename("copd"="characteristics_ch1.2") %>%
  mutate(copd=case_when(
    copd == "copd status: yes" ~ "COPD",
