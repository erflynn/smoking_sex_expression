...
This is the example 
```{r}
gse.obj <- GEOquery::getGEO("GSE108134") 
pData <- pData(gse.obj[[1]]) 
pData_sm <- pData(gse.obj[[1]]) %>% 
  select(geo_accession, 
         source_name_ch1, 
         "30-day mean pm2.5 exposure level:ch1",
         "smoking status:ch1",
         "time of serial bronchoscopy (month):ch1",
         "description",
         "characteristics_ch1")
...
pData_sm4 <- pData_sm3 %>% 
  ...
  mutate(smok=case_when(
    str_detect(smok, "nonsmoker") ~ "NS",
    str_detect(smok, "COPD smoker") ~ "S",
    str_detect(smok, "smoker") ~ "S"
    #TRUE ~ smok_col
  ))

View(pData_sm4)
write.csv(pData_sm4, "gse108134.csv")
```
